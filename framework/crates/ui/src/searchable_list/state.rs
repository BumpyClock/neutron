    ...
    <D::Item as SearchableListItem>::Value: PartialEq + Clone,
{
    ...
    pub(crate) empty: Option<Box<dyn Fn(&mut Window, &App) -> Option<AnyElement> + 'static>>,
    ...
    <D::Item as SearchableListItem>::Value: PartialEq + Clone,
{
    ...
    pub fn new<P: 'static>(
        ...
        Self::new_with_empty(
            ...
            Some(Box::new(move |window, cx| {
                Some(on_render_empty(window, cx))
            })),
            on_blur,
            window,
            cx,
    ...
    /// Create a state with an optional component-specific empty-state renderer.
    #[allow(clippy::too_many_arguments)]
    pub(crate) fn new_with_empty<P: 'static>(
        delegate: D,
        selected_indices: Vec<IndexPath>,
        on_confirm: impl Fn(
            Option<IndexPath>,
        ...
        let adapter =
            SearchableListAdapter::new_with_empty(delegate, on_confirm, on_cancel, on_render_empty);
        let list = cx.new(|cx| {
            ListState::new(adapter, window, cx)
...
mod tests {
    use super::*;
    use crate::list::List;
    use gpui::{IntoElement, Modifiers, Render, TestAppContext, VisualTestContext, div, point, px};

    struct HoverHarness {
        state: SearchableListState<Vec<&'static str>>,
    }
    ...
    fn on_blur(_: &mut HoverHarness, _: &mut Window, _: &mut Context<HoverHarness>) {}

    #[gpui::test]
    fn rendered_hover_updates_cursor(cx: &mut TestAppContext) {
        let window = cx.update(|cx| {
            crate::init(cx);
            cx.open_window(Default::default(), |window, cx| {
                cx.new(|cx| HoverHarness {
                    state: SearchableListState::new(
        ...
        assert_eq!(
            window
                .read_with(&visual_cx, |fixture, cx| {
                    fixture.state.list.read(cx).selected_index()
                })
                .unwrap(),
        ...
        for y in 0..200 {
            ...
            let selected = window
                ...
                .unwrap();
            if selected == Some(IndexPath::new(1)) {
                hovered_row = Some(y);
                break;
            }
        }

        assert!(hovered_row.is_some(), "rendered hover must select row 1");
    }
}
