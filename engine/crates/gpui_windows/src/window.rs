...
pub struct WindowsWindowState {
    ...
    /// Set when the next draw must bypass cached scene data after recovery or deferral.
...
fn retrieve_window_placement(
    ...
    // CW_USEDEFAULT can place the window on a different monitor. These logical bounds
    // belong to the target display, not the window's initial monitor.
