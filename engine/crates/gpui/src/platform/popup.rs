...
mod tests {
    ...
    #[test]
    fn popup_anchor_and_gravity_default_to_center() {
        assert_eq!(PopupAnchor::default(), PopupAnchor::Center);
        assert_eq!(PopupGravity::default(), PopupGravity::Center);
    }

    #[test]
    fn popup_constraint_adjustment_uses_platform_bit_values() {
        assert!(PopupConstraintAdjustment::default().is_empty());
        assert_eq!(PopupConstraintAdjustment::SLIDE_X.bits(), 1);
        assert_eq!(PopupConstraintAdjustment::SLIDE_Y.bits(), 2);
        assert_eq!(PopupConstraintAdjustment::FLIP_X.bits(), 4);
        assert_eq!(PopupConstraintAdjustment::FLIP_Y.bits(), 8);
        assert_eq!(PopupConstraintAdjustment::RESIZE_X.bits(), 16);
        assert_eq!(PopupConstraintAdjustment::RESIZE_Y.bits(), 32);

        let adjustments = PopupConstraintAdjustment::SLIDE_X
            | PopupConstraintAdjustment::FLIP_Y
            | PopupConstraintAdjustment::RESIZE_X;
        assert!(adjustments.contains(PopupConstraintAdjustment::SLIDE_X));
        assert!(adjustments.contains(PopupConstraintAdjustment::FLIP_Y));
        assert!(adjustments.contains(PopupConstraintAdjustment::RESIZE_X));
        assert!(!adjustments.contains(PopupConstraintAdjustment::SLIDE_Y));
    }
}
