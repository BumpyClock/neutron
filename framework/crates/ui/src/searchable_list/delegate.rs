...
pub trait SearchableListDelegate: Sized + 'static {
    ...
    /// Context-aware search hook for compatibility adapters.
    ///
    /// The default forwards to [`Self::perform_search`], while adapters can use the concrete
    /// owner context to preserve an older delegate contract.
    ...
    ///
    /// When `Some(_)` is returned, the adapter keeps its `SearchableListItemElement` wrapper for
    /// size, hover, and accessibility behavior, but replaces default row content and check icon.
    /// Return `None` to fall back to standard content and check-icon rendering.
    ///
    ...
    /// Called when the dropdown/popover closes through item selection, Escape, focus loss,
    /// trigger or outside click, or explicit confirm. `final_selection` is the selection after
